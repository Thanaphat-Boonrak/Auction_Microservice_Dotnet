...
const renderer = ({
  ...
  return (
    <div
      className={`border-2 border-white text-white py-1 px-2 rounded-lg flex justify-center ${completed ? "bg-red-600" : days === 0 && hours < 10 ? "bg-amber-600" : "bg-green-600"} `}
    >
      {completed ? (
...
export default function CountdownTimer({ auctionEnd }: Props) {
  ...
  const auctionFinished = () => {
    if (pathname.includes("/auctions/details")) {
      setOpen(false);
    }
  };
  return (
